...
<head>
  ...
  <style>
    :root {
      --bg: #04080f;
      --surface: rgba(7, 13, 24, 0.88);
      --surface-strong: rgba(10, 16, 29, 0.96);
      ...
      --muted: #9eb1c7;
      --radius: 28px;
      --shadow: 0 40px 80px rgba(4, 10, 20, 0.38);
    ...
    * {
    ...
    body {
      ...
      color: var(--text);
      background: radial-gradient(circle at top, rgba(34, 197, 94, 0.05), transparent 22%), linear-gradient(180deg, #02050a 0%, #060b14 100%);
    }
    ...
    body::before {
      ...
      position: fixed;
      inset: 0;
      background: radial-gradient(circle at 18% 18%, rgba(34, 197, 94, 0.12), transparent 14%), radial-gradient(circle at 82% 15%, rgba(34, 197, 94, 0.08), transparent 10%);
      pointer-events: none;
      z-index: 0;
    }
    ...
    <section class="header-panel">
      <div class="header-copy">
        <div>
          ...
          <p>Seu centro de controle Industrial IoT está ativo. Acompanhe seus dispositivos de gás e conecte seu primeiro
            projeto com confiança.</p>
        </div>
      </div>
      ...
      <div class="content-grid">
        <div class="panel">
          ...
          <p>Você está autenticado com o email <?php echo escapeHtml($currentUser->getemail()); ?>. Conecte dispositivos
            e visualize o status em tempo real.</p>
        </div>

        <div class="panel">
          <h2>Atividades recentes</h2>
          <div class="status-card">
            <p style="margin:0;color:var(--muted);">Nenhum projeto conectado ainda? Comece com a configuração do seu
              primeiro dispositivo.</p>
          </div>
        </div>
      </div>
            ...
            <?php foreach ($projects as $project): ?>
              <div class="project-card">
                ...
                <div style="display:grid;gap:10px;">
                  ...
                  <p style="margin:0;color:var(--muted);">Última comunicação:
                    <?php echo $project->getLastOnline() ? date('d/m/Y H:i', strtotime($project->getLastOnline())) : 'Ainda não sincronizado'; ?>
                  </p>
                  <p style="margin:0;color:var(--muted);">Firmware:
                    <?php echo escapeHtml($project->getFirmwareVersion() ?: 'Aguardando'); ?></p>
                </div>
              </div>
            <?php endforeach; ?>
          </div>
      ...
      <?php else: ?>
        <div class="panel empty-state">
          ...
          <button class="primary-button" onclick="location.href='project-connect.php'">Conectar seu primeiro
            projeto</button>
        </div>
      <?php endif; ?>
    </section>
