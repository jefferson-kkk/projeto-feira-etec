...
<head>
  ...
  <style>
    :root { color-scheme: dark; --bg: #04080f; --surface: rgba(7, 13, 24, 0.94); --surface-2: rgba(255,255,255,0.04); --border: rgba(255,255,255,0.08); --accent: #22c55e; --accent-soft: rgba(34,197,94,0.14); --text: #eef3f8; --muted: #9eb1c7; --danger: #f87171; --radius: 24px; --ease: cubic-bezier(0.4,0,0.2,1); }
    * { box-sizing: border-box; }
    html, body { margin: 0; min-height: 100%; }
    body { font-family: 'Inter', sans-serif; color: var(--text); background: radial-gradient(circle at top, rgba(34,197,94,0.06), transparent 22%), linear-gradient(180deg, #02050a 0%, #060b14 100%); }
    body::before { content: ''; position: fixed; inset: 0; background: radial-gradient(circle at 18% 16%, rgba(34,197,94,0.12), transparent 14%); pointer-events: none; }
    a { color: inherit; text-decoration: none; }
    .page { position: relative; z-index: 1; width: min(1120px, calc(100% - 32px)); margin: 0 auto; padding: 28px 0 44px; }
    .topbar { display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 24px; }
    .brand { display: flex; align-items: center; gap: 12px; }
    .brand-circle { width: 44px; height: 44px; border-radius: 14px; display: grid; place-items: center; background: var(--accent-soft); color: var(--accent); font-weight: 800; }
    .brand span { display: block; color: var(--muted); font-size: 12px; letter-spacing: .16em; text-transform: uppercase; }
    .brand strong, h1, h2 { font-family: 'Sora', sans-serif; }
    .top-actions { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; justify-content: flex-end; }
    .button { border: 0; border-radius: 14px; padding: 12px 16px; display: inline-flex; align-items: center; justify-content: center; gap: 8px; background: linear-gradient(135deg, #22c55e, #16a34a); color: #071f12; font-weight: 800; cursor: pointer; transition: transform .2s var(--ease), opacity .2s var(--ease); }
    .button:hover { transform: translateY(-1px); }
    .button.secondary { background: rgba(255,255,255,0.06); color: var(--text); border: 1px solid var(--border); }
    .button.danger { background: rgba(248,113,113,0.12); color: #fecaca; border: 1px solid rgba(248,113,113,0.2); }
    .button[disabled] { opacity: .62; cursor: wait; transform: none; }
    .hero { display: grid; grid-template-columns: 320px minmax(0, 1fr); gap: 22px; align-items: stretch; margin-bottom: 22px; }
    .card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 24px; box-shadow: 0 30px 80px rgba(0,0,0,.24); backdrop-filter: blur(24px); }
    .identity { display: grid; justify-items: center; text-align: center; gap: 14px; }
    .avatar-wrap { position: relative; width: 168px; height: 168px; border: 1px solid rgba(255,255,255,.12); border-radius: 50%; overflow: hidden; background: rgba(34,197,94,.14); display: grid; place-items: center; cursor: pointer; }
    .avatar-wrap img { width: 100%; height: 100%; object-fit: cover; }
    .avatar-initials { font-size: 46px; font-weight: 800; color: var(--accent); }
    .avatar-overlay { position: absolute; inset: 0; display: grid; place-items: center; background: rgba(2,6,12,.62); color: #fff; opacity: 0; transition: opacity .2s var(--ease); font-weight: 800; }
    .avatar-wrap:hover .avatar-overlay { opacity: 1; }
    .identity h1 { margin: 8px 0 0; font-size: 28px; }
    .identity p { margin: 0; color: var(--muted); }
    .online { display: inline-flex; align-items: center; gap: 8px; color: #bdf7d0; background: var(--accent-soft); padding: 7px 12px; border-radius: 999px; font-size: 13px; font-weight: 700; }
    .online::before { content: ''; width: 8px; height: 8px; border-radius: 50%; background: var(--accent); box-shadow: 0 0 0 5px rgba(34,197,94,.1); }
    .summary { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }
    .summary-item { background: var(--surface-2); border: 1px solid var(--border); border-radius: 18px; padding: 18px; }
    .summary-item span { display: block; color: var(--muted); font-size: 13px; margin-bottom: 8px; }
    .summary-item strong { font-size: 15px; overflow-wrap: anywhere; }
    .tabs { display: flex; gap: 8px; flex-wrap: wrap; margin: 22px 0; }
    .tab { border: 1px solid var(--border); background: rgba(255,255,255,.04); color: var(--muted); padding: 11px 14px; border-radius: 14px; cursor: pointer; font-weight: 800; }
    .tab.active { color: var(--text); background: var(--accent-soft); border-color: rgba(34,197,94,.28); }
    ...
    input, textarea, select { width: 100%; border-radius: 14px; border: 1px solid rgba(255,255,255,.08); background: rgba(255,255,255,.05); color: var(--text); padding: 13px 14px; font: inherit; outline: none; }
    textarea { min-height: 110px; resize: vertical; }
    input:focus, textarea:focus, select:focus { border-color: rgba(34,197,94,.55); box-shadow: 0 0 0 5px rgba(34,197,94,.08); }
    .info-list { display: grid; gap: 12px; }
    .info-row { display: grid; grid-template-columns: 180px 1fr; gap: 12px; padding: 14px 0; border-bottom: 1px solid var(--border); }
    .info-row:last-child { border-bottom: 0; }
    .label { color: var(--muted); }
    .value { overflow-wrap: anywhere; }
    .switch-list { display: grid; gap: 12px; }
    .switch-row { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 16px; border: 1px solid var(--border); border-radius: 18px; background: var(--surface-2); }
    ...
    .strength { height: 8px; border-radius: 999px; background: rgba(255,255,255,.08); overflow: hidden; }
    .strength span { display: block; height: 100%; width: 0; background: var(--danger); transition: width .2s var(--ease), background .2s var(--ease); }
    .session-card { display: grid; gap: 12px; border: 1px solid var(--border); border-radius: 18px; padding: 18px; background: var(--surface-2); }
    .toast { position: fixed; right: 22px; bottom: 22px; z-index: 30; min-width: 260px; max-width: min(420px, calc(100vw - 32px)); padding: 14px 16px; border-radius: 16px; border: 1px solid rgba(34,197,94,.24); background: rgba(7,13,24,.96); color: var(--text); box-shadow: 0 24px 60px rgba(0,0,0,.3); opacity: 0; transform: translateY(12px); pointer-events: none; transition: opacity .24s var(--ease), transform .24s var(--ease); }
    .toast.show { opacity: 1; transform: translateY(0); }
    .toast.error { border-color: rgba(248,113,113,.28); color: #fecaca; }
    .viewer, .cropper { position: fixed; inset: 0; z-index: 40; display: none; align-items: center; justify-content: center; background: rgba(1,4,9,.78); backdrop-filter: blur(14px); padding: 24px; }
    .viewer.open, .cropper.open { display: flex; }
    .viewer img { max-width: min(88vw, 920px); max-height: 82vh; border-radius: 20px; cursor: grab; transition: transform .18s var(--ease); box-shadow: 0 40px 100px rgba(0,0,0,.5); }
    .close { position: fixed; top: 22px; right: 22px; width: 44px; height: 44px; border-radius: 50%; border: 1px solid var(--border); background: rgba(255,255,255,.08); color: var(--text); cursor: pointer; font-size: 22px; }
    .crop-card { width: min(560px, 100%); background: var(--surface); border: 1px solid var(--border); border-radius: 22px; padding: 20px; display: grid; gap: 16px; }
    .crop-stage { position: relative; width: 100%; aspect-ratio: 1; overflow: hidden; border-radius: 18px; background: #02050a; border: 1px solid var(--border); cursor: grab; }
    .crop-stage img { position: absolute; top: 50%; left: 50%; max-width: none; transform-origin: center; user-select: none; pointer-events: none; }
    .crop-actions { display: flex; justify-content: flex-end; gap: 10px; flex-wrap: wrap; }
    .hidden-file { display: none; }
    @media (max-width: 860px) { .hero, .grid { grid-template-columns: 1fr; } .summary { grid-template-columns: 1fr; } .info-row { grid-template-columns: 1fr; } .page { width: min(100% - 24px, 1120px); padding-top: 18px; } .topbar { align-items: flex-start; flex-direction: column; } }
  </style>
</head>
<body>
  <main class="page">
    ...
    <section class="hero">
      ...
      <div class="card">
        ...
        <section class="section active" id="profile">
          <div class="info-list">
            <div class="info-row"><div class="label">Nome completo</div><div class="value"><?php echo escapeHtml($currentUser->getnome()); ?></div></div>
            <div class="info-row"><div class="label">Username</div><div class="value">@<?php echo escapeHtml($currentUser->getusername()); ?></div></div>
            <div class="info-row"><div class="label">E-mail</div><div class="value"><?php echo escapeHtml($currentUser->getemail()); ?></div></div>
            <div class="info-row"><div class="label">Telefone</div><div class="value"><?php echo escapeHtml($currentUser->getphone() ?: 'Não informado'); ?></div></div>
            <div class="info-row"><div class="label">Conta criada em</div><div class="value"><?php echo date('d/m/Y H:i', strtotime($currentUser->getdatacriacao())); ?></div></div>
            <div class="info-row"><div class="label">Membro desde</div><div class="value"><?php echo date('d/m/Y', strtotime($memberSince)); ?></div></div>
          </div>
        </section>
        ...
        <section class="section" id="settings">
          <form class="ajax-form grid" data-action="update_profile">
            ...
            <div class="field"><label>Language</label><select name="language"><option value="pt-BR" <?php echo $currentUser->getlanguage() === 'pt-BR' ? 'selected' : ''; ?>>Português (Brasil)</option><option value="en" <?php echo $currentUser->getlanguage() === 'en' ? 'selected' : ''; ?>>English</option><option value="es" <?php echo $currentUser->getlanguage() === 'es' ? 'selected' : ''; ?>>Español</option></select></div>
            <div class="field full"><label>Timezone</label><select name="timezone"><option value="America/Sao_Paulo" <?php echo $currentUser->gettimezone() === 'America/Sao_Paulo' ? 'selected' : ''; ?>>America/Sao_Paulo</option><option value="UTC" <?php echo $currentUser->gettimezone() === 'UTC' ? 'selected' : ''; ?>>UTC</option><option value="America/New_York" <?php echo $currentUser->gettimezone() === 'America/New_York' ? 'selected' : ''; ?>>America/New_York</option></select></div>
        ...
        <section class="section" id="security">
          <form class="ajax-form grid" data-action="change_password">
            ...
            <div class="field full"><label>Força da senha</label><div class="strength"><span id="strengthBar"></span></div></div>
        ...
        <section class="section" id="sessions">
          <div class="session-card">
            ...
            <div class="info-list">
              <div class="info-row"><div class="label">Browser</div><div class="value"><?php echo escapeHtml($browser); ?></div></div>
              <div class="info-row"><div class="label">Sistema operacional</div><div class="value"><?php echo escapeHtml($os); ?></div></div>
              <div class="info-row"><div class="label">IP Address</div><div class="value"><?php echo escapeHtml($ipAddress); ?></div></div>
              <div class="info-row"><div class="label">Última atividade</div><div class="value"><?php echo date('d/m/Y H:i'); ?></div></div>
            </div>
            <div class="top-actions">
  ...
  <script>
    ...
    cropStage.addEventListener('pointerdown', event => {
      dragState = { x: event.clientX, y: event.clientY, startX: cropX, startY: cropY };
    ...
    viewer.addEventListener('pointerdown', event => {
      ...
      dragState = { x: event.clientX, y: event.clientY, startX: viewerX, startY: viewerY };
    ...
    viewer.addEventListener('pointerup', () => { dragState = null; });
