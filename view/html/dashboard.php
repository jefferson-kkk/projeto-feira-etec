...
/* margin-top: o botao #sidebarToggle e fixed em top:14/left:14 (ele
   precisa ficar acessivel tanto com a sidebar aberta quanto fechada,
   ja que e o unico controle dos dois estados) -- sem essa folga, ele
   ficava exatamente em cima da logo. */
.brand{padding:4px 8px 14px;margin-top:30px;gap:12px;border-bottom:1px solid rgba(229,255,243,.06)}
...
.chat-convo-row{position:relative;display:flex;align-items:center}
.chat-convo-row .chat-convo{flex:1;min-width:0;padding-right:30px}
.convo-delete{position:absolute;right:6px;top:50%;transform:translateY(-50%);width:22px;height:22px;border:0;border-radius:50%;background:transparent;color:var(--muted);font-size:16px;line-height:1;font-weight:400;cursor:pointer;display:flex;align-items:center;justify-content:center;opacity:0;transition:opacity .15s,background .15s,color .15s}
.chat-convo-row:hover .convo-delete{opacity:1}
.convo-delete:hover{background:rgba(255,87,87,.14);color:#ff8585}
.convo-delete:focus-visible{opacity:1;outline:2px solid rgba(255,87,87,.5);outline-offset:1px}
@media(hover:none){.convo-delete{opacity:1}}
...
.chat-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:14px 18px;border-bottom:1px solid var(--border);flex-shrink:0}
...
/* min-height:0 é essencial aqui: sem ele, um item flex assume
   min-height:auto por padrao e cresce para caber TODO o conteudo em
   vez de manter a altura do .chat-main e deixar o overflow-y rolar --
   como o .chat-shell pai corta o que passa da altura (overflow:hidden),
   mensagens antigas ficavam cortadas e inacessiveis em vez de roláveis. */
.chat-body{flex:1;min-height:0;overflow-y:auto;padding:28px 30px}
.assist-messages,.messages{display:flex;flex-direction:column;gap:34px;max-width:820px;margin:0 auto}
/* margin-bottom aqui (nao so o gap do wrapper acima) porque
   renderChat() injeta as bolhas direto dentro de #chatBody, sem
   nenhum elemento .assist-messages/.messages envolvendo elas -- ou
   seja, o gap do flex-wrapper acima nunca chega a se aplicar de
   verdade nessa tela. */
.chat-bubble{display:flex;gap:11px;max-width:74%;margin-bottom:30px}
...
.bubble-body{background:rgba(255,255,255,.045);border:1px solid var(--border);border-radius:16px;padding:14px 18px;font-size:13.5px;line-height:1.7;white-space:pre-wrap}
...
.chat-main .compose{border-top:1px solid var(--border);padding:12px 16px;margin:0;flex-shrink:0}
...
<aside class="sidebar">
  <a class="brand" href="#dashboard" data-view="dashboard"><img class="brand-mark" src="/view/html/sadag-mark.png" alt="Sadag"><span><small>Monitoramento IoT</small><strong>Sadag</strong></span></a>
  <nav class="nav">
...
function messageBubble(m){const cls=m.sender_type==='user'?'me':(m.sender_type==='support'?'team':'ai');const who=m.sender_type==='user'?'Você':(m.sender_type==='support'?'Equipe Sadag':'Sadag Assist');return `<div class="chat-bubble ${cls}${m._arrived?' arrived':''}"><div class="bubble-body">${esc(m.message)}<small>${who} · ${fmtDate(m.created_at)}${Number(m.email_sent)?' · email enviado':''}</small></div></div>`}
function renderChat(){const box=document.getElementById('chatBody');const empty=!state.ticketId&&!state.messages.length;const greet=empty?'<div class="chat-bubble ai"><div class="bubble-body">Olá! Sou o Sadag Assist — estou presente em todas as suas conversas. Como posso ajudar?<small>Sadag Assist</small></div></div>':'';const bubbles=state.messages.map(messageBubble).join('');const typing=state.thinking?'<div class="chat-bubble ai typing"><div class="bubble-body"><span class="thinking-text">Sadag Assist está pensando...</span></div></div>':'';box.innerHTML=greet+bubbles+typing+(empty?ASSIST_SUGGESTIONS_HTML:'');box.scrollTop=box.scrollHeight;box.querySelectorAll('.arrived').forEach(el=>setTimeout(()=>el.classList.remove('arrived'),2200))}
function renderConvoList(){const box=document.getElementById('convoList');const novo=`<button type="button" class="chat-convo ${!state.ticketId?'active':''}" data-new="1"><span class="convo-avatar">🤖</span><span class="convo-info"><strong>Nova conversa</strong></span></button>`;const items=state.tickets.map(t=>`<div class="chat-convo-row"><button type="button" class="chat-convo ${Number(t.id)===Number(state.ticketId)?'active':''}" data-ticket="${t.id}" title="${esc(t.subject)}"><span class="convo-avatar">${Number(t.escalated)?'🛠️':'🤖'}</span><span class="convo-info"><strong>${esc(t.subject)}</strong></span></button><button type="button" class="convo-delete" data-delete-ticket="${t.id}" title="Apagar conversa" aria-label="Apagar conversa">×</button></div>`).join('');box.innerHTML=novo+items}
async function deleteTicket(id){const t=state.tickets.find(x=>Number(x.id)===Number(id));if(!t)return;if(!confirm(`Apagar a conversa "${t.subject}"? Todas as mensagens dela serão perdidas. Essa ação não pode ser desfeita.`))return;try{await api('app.php?action=support_delete',{method:'POST',body:JSON.stringify({ticket_id:id})});if(Number(state.ticketId)===Number(id)){state.ticketId=null;state.messages=[];renderChat()}await loadTickets();updateChatHeader();toast('Conversa apagada.')}catch(e){toast(e.message)}}
